let healthData = JSON.parse(localStorage.getItem('health'))
...
const sorter = () =>{
    ...
    if(select.value == 'l2h'){
        healthData.sort((a,b)=>Number(a.price)-Number(b.price));
        displayData(healthData)
    }
    if(select.value == 'h2l'){
        healthData.sort((a,b)=>Number(b.price)-Number(a.price));
        displayData(healthData)
    }
}